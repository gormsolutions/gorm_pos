...
class BranchExpenses(Document):
    def on_submit(self):
        # Check for existing Journal Entry
        existing_receipt = frappe.get_all("Journal Entry", filters={"custom_branch_expense_id": self.name}, limit=1)
        
        if existing_receipt:
            frappe.msgprint(_(f"Branch Expenses already exists for {self.name}"))
            return
        ...
        frappe.db.commit()
        frappe.msgprint(_(f"Successfully Posted  Branch Expenses for {self.name}"))
