...
#     gas_warehouses = [w for w in allowed_warehouses if "gas" in w.lower()]
#     empty_warehouses = [w for w in allowed_warehouses if "empty" in w.lower()]
#     other_warehouses = [w for w in allowed_warehouses if w not in gas_warehouses + empty_warehouses]
...
#         if bundle_name:
#             # ✅ Bundled items — use only gas warehouses
...
#                 for warehouse in gas_warehouses:
...
#         else:
#             # ✅ Regular items — use different warehouse groups based on name
#             item_name_lower = item["item_name"].lower()

#             if "gas" in item_name_lower:
#                 selected_warehouses = gas_warehouses
#             elif "empty" in item_name_lower:
#                 selected_warehouses = empty_warehouses
#             else:
#                 selected_warehouses = gas_warehouses
...
# ✅ Helper: Get latest qty_after_transaction from Stock Ledger Entry
def get_stock_from_ledger(item_code, warehouse):
...
def get_gasitems_details_buddle(limit, offset, search=None, user=None):
    ...
    for i, item in enumerate(item_details):
        ...
        if bundle_name:
            # ✅ Bundled items — use gas warehouses only
            bundle_items_raw = frappe.get_all(
                "Product Bundle Item",
                filters={"parent": bundle_name},
                fields=["item_code"]
            ...
            for b in bundle_items_raw:
                ...
                for warehouse in gas_warehouses:
                    stock_balance = get_stock_from_ledger(b["item_code"], warehouse)
                    component_stock += stock_balance
                    warehouses.append({
                        "warehouse_name": warehouse,
                        "stock": stock_balance
            ...
            # ✅ Regular items — determine warehouse group
            item_name_lower = item["item_name"].lower()
            if "gas" in item_name_lower:
                selected_warehouses = gas_warehouses
            elif "empty" in item_name_lower:
            ...
            for warehouse in selected_warehouses:
                stock_balance = get_stock_from_ledger(item["item_code"], warehouse)
                stock_total += stock_balance
                warehouse_stock.append({
                    "warehouse_name": warehouse,
                    "stock": stock_balance
        ...
        # ✅ Add original_price after custom_on_promotion
        item_ordered = {}
        for key in item:
            item_ordered[key] = item[key]
            if key == "custom_on_promotion":
