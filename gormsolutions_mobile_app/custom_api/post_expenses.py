...
def create_self_expense(items=None, posting_date=None):
    ...
    expense_setting = frappe.get_value("Expense Settings", {"user": user}, ["party", "party_type", "company", "payable_account", "mode_of_payment"], as_dict=True)
    ...
    child_items = []
    for item in items:
        if not item.get("amount") or not item.get("party"):
            frappe.throw(_("Each item must include at least 'party' and 'amount'."))
        child_items.append({
    ...
    # Create and submit the Branch Expenses document branch
